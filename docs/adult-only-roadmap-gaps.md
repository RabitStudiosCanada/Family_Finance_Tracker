...
1. **Ship in-app CRUD flows for credit cards & income streams**  
   _Roadmap reference:_ Phase 1 “Add/edit/archive cards” and “Add/edit income sources.”  
   _Why it’s outstanding:_ The current tables only render read-only data and tell users to “Use the API to add one,” so adults cannot manage their own financial instruments without external tooling.【F:frontend/src/pages/CreditCardsPage.jsx†L19-L76】【F:frontend/src/pages/IncomeStreamsPage.jsx†L28-L79】  
   _Key work:_ Extend the finance API client with POST/PATCH helpers, add modal forms for create/update, surface archive toggles, and hook mutations into the existing query cache.

2. **Expose projected expense planning in the UI**  
   _Roadmap reference:_ Phase 2 “Projected expenses (backend APIs & seeds).”  
   _Why it’s outstanding:_ Backend lifecycle endpoints (plan → commit → paid/cancel) exist, but the React client has no routes or API bindings for managing projections, so adults cannot stage upcoming obligations from the app.【F:backend/src/services/projectedExpensesService.js†L1-L178】【F:frontend/src/api/finance.js†L1-L68】  
   _Key work:_ Add projected-expense API client, create planning screens (list, filters, detail drawer), support status transitions, and surface upcoming projections on the overview page.

3. **Deliver savings goal management with progress visuals**  
...
4. **Implement category budget tracking and warnings**  
...
5. **Add reusable projected expense templates (grocery list)**  
