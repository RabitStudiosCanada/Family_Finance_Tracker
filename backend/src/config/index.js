...
const envSchema = z
  .object({
    ...
    JWT_ACCESS_SECRET: z.string().min(8, 'JWT access secret must be at least 8 characters').optional(),
...
const parseEnvironment = () => {
  ...
  const jwtAccessSecret =
    parsed.data.JWT_ACCESS_SECRET ?? (env === 'production' ? undefined : 'change-me-access-secret');
  const jwtRefreshSecret =
    parsed.data.JWT_REFRESH_SECRET ?? (env === 'production' ? undefined : 'change-me-refresh-secret');

  if (!jwtAccessSecret) {
    throw new Error('JWT_ACCESS_SECRET must be provided in production environments');
  }

  if (!jwtRefreshSecret) {
    throw new Error('JWT_REFRESH_SECRET must be provided in production environments');
  }

  const database = buildDatabaseConfig(env, parsed.data.DATABASE_CLIENT, parsed.data.DATABASE_URL);
